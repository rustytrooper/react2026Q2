...
// hooks/useFetchCharacters/useFetchCharacters.ts
'use client'
...
export function useDisneyData() {
  ...
  // Адаптация: читаем параметры как в Next.js
  const currentPage = parseInt(searchParams?.get('page') || '1', 10);
  const searchQueryFromURL = searchParams?.get('query') || '';
  ...
  // Создаем функцию, имитирующую setSearchParams из React Router
  const setSearchParams = useCallback(
    (updater: (prev: URLSearchParams) => URLSearchParams) => {
      ...
      // Используем replace вместо push, чтобы не создавать историю
      router.replace(url!);
    },
